#!/usr/bin/env bash
# Crow shared metrics — JSONL append with atomic mkdir locks + 10MB rotation

# Source constants if not already loaded
if [[ -z "${CROW_LOCK_SUFFIX:-}" ]]; then
...
log_metric() {
  ...
  local payload="$2"
  local lock_dir="${file}${CROW_LOCK_SUFFIX}"
  local max_size="${CROW_MAX_METRICS_BYTES:-10485760}"
